...
class FoodManagementProvider {
  static final FoodManagementProvider _instance = FoodManagementProvider._internal();
  ...
  final String _baseUrl = 'https://test-prod-f427.onrender.com'; // Replace with your actual base URL
    ...
    try {
      final response = await http.get(
        ...
        headers: {
          ...
          'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJkYjJhMGI0YS1kYTNjLTRiNjQtOTYxNS0yYmIwOTBmYzg1OTEiLCJtb2JpbGUiOiIrOTE3ODQyOTAwMTU1IiwiaWF0IjoxNzQ0Nzk5NjU3LCJleHAiOjE3NDQ4MDMyNTd9.Dta2g1j_Z4AvJiGEHuxBf8w6u7M5x1N5V6VLpqpp-KU',
        },
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          ...
          throw Exception(responseData['message'] ?? 'Failed to fetch food data');
        }
      } else {
        throw Exception('Failed to fetch food data. Status: ${response.statusCode}');
      }
    } catch (error) {
  ...
  Map<String, List<Map<String, dynamic>>> _processApiMealData(Map<String, dynamic> apiData) {
        ...
        if (result.containsKey(mealType)) {
          for (var foodItem in foodList) {
            ...
            Map<String, dynamic> processedFood = {
              ...
              'calories': foodItem['calories'] is int ? foodItem['calories'] : (foodItem['calories'] as double).toInt(),
              ...
              'image': foodItem['imageUrl'] ?? _getDefaultImageForMealType(mealType),
            };
  ...
  List<Map<String, dynamic>> getAllFoodsSortedByTime(Map<String, List<Map<String, dynamic>>> mealData) {
  ...
  int getTotalCaloriesForTimePeriod(
      Map<String, List<Map<String, dynamic>>> mealData, 
      Map<String, dynamic> timePeriod) {
    List<Map<String, dynamic>> periodFoods = getFoodsForTimePeriod(mealData, timePeriod);
  ...
  Map<String, double> calculateTotalNutrients(Map<String, List<Map<String, dynamic>>> mealData, [String? specificMealType]) {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // debugPrint(responseData); // Debugging line to check the response data
      
      // Transform API data to match the format of your _popularFoods structure
      return responseData.map((item) {
        // Calculate total calories (using energy_kcal from macronutrients)
        final int calories = (item['macronutrients']['energy_kcal'] is int) 
            ? item['macronutrients']['energy_kcal'] 
            : (item['macronutrients']['energy_kcal'] as double).toInt();
            
        // Get the first ingredient as a fallback for image
        // final String firstIngredient = (item['ingredients'] != null && 
        ...
        return {
          ...
          // // Add additional nutritional information that might be useful
          // 'protein': item['macronutrients']['protein_g'],
          // 'carbs': item['macronutrients']['carbohydrates_g'],
          // 'fat': item['macronutrients']['fat_g'],
          // 'fiber': item['macronutrients']['fiber_g'],
        };
      }).toList();
    } else {
      throw Exception('Failed to fetch popular foods. Status: ${response.statusCode}');
    }
  } catch (error) {
    print('Error fetching popular foods: $error');
    // Return dummy data as fallback when API fails
    // return _getDummyPopularFoods();
...
Future<String> _getAccessToken() async {
  ...
  return 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiJkYjJhMGI0YS1kYTNjLTRiNjQtOTYxNS0yYmIwOTBmYzg1OTEiLCJtb2JpbGUiOiIrOTE3ODQyOTAwMTU1IiwiaWF0IjoxNzQ0Nzk5NjU3LCJleHAiOjE3NDQ4MDMyNTd9.Dta2g1j_Z4AvJiGEHuxBf8w6u7M5x1N5V6VLpqpp-KU';
}
...
IconData _getIconForFoodItem(String foodName) {
  ...
  if (name.contains('salad') || name.contains('vegetable') || name.contains('spinach')) {
    return Icons.eco;
  } else if (name.contains('fruit') || name.contains('apple') || name.contains('banana')) {
    return Icons.breakfast_dining;
  } else if (name.contains('meat') || name.contains('chicken') || name.contains('beef')) {
    return Icons.dinner_dining;
  } else if (name.contains('drink') || name.contains('juice') || name.contains('smoothie')) {
