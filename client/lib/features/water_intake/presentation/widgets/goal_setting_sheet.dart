...
class _GoalSettingSheetState extends State<GoalSettingSheet> {
  ...
  Future<void> _loadCurrentGoal() async {
    try {
      final profile = await Provider.of<WaterIntakeApiProvider>(context, listen: false)
          .getUserProfile();
      if (profile['target_water_ml'] != null) {
        setState(() {
          _selectedGoal = profile['target_water_ml'].toDouble();
          _initialized = true;
        });
      }
    } catch (e) {
      debugPrint('Error loading current goal: $e');
  ...
  Future<void> _saveGoal() async {
    ...
    try {
      ...
      await apiProvider.updateWaterIntakeTarget(_selectedGoal.toInt());
